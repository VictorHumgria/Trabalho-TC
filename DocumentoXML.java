    ...
    public void concatenacao(Automato auto1,Automato auto2, String patch){
        try {
            ...
            for(int k=0; k<auto2.getState().size(); k++){
                ...
                String finalAuto2 = "1" + auto2.getState().get(k).getId();
                String estadoFinal = "1" + auto2.estadoFinal;

                if(finalAuto2.equalsIgnoreCase(estadoFinal)){       // como esse é o autômato 2 que ficará no final ele pesquisa o id do estado final do automato2 para definir como final da concatenação;                    
            ...
            for(int j = 0; j < auto1.getTransition().size(); j++) {
                ...
                Element read = doc.createElement("read");                                                  // cria o elemento Read para a transicao
                                                                  
                if (j == auto1.getTransition().size() - 2) {                                                       // aqui verifica se é o ultimo estado do automato que esta sendo lido, se for é criado a ligação lambda
                    from.appendChild(doc.createTextNode(auto1.getState().get(contStateauto1).getId()));            // se não simplesmente monta o estado de acordo com os dados do arrayList;
                    transition.appendChild(from);
                    to.appendChild(doc.createTextNode("1"+auto2.getState().get(0).getId()));
                    ...
                    if(auto1.getTransition().get(j).getRead().equalsIgnoreCase("lambda")){
                        read.appendChild(doc.createTextNode(""));
                        transition.appendChild(read);
                    }else{
